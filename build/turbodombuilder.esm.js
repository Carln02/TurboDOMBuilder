...
class Transition {
     ...
     */
    initialize(direction = InOut.out, elements, executeForAll = false, recomputeIndices = true, overrideStyles, overrideBeforeComputing = this.beforeComputing) {
     ...
     * @returns {this} Itself for method chaining.
     */
    apply(direction = InOut.out, elements, executeForAll = false, recomputeIndices = true, overrideStyles, overrideBeforeComputing = this.beforeComputing) {
        this.getEnabledEntriesData(elements, executeForAll, recomputeIndices, overrideBeforeComputing)
            .forEach((data) => {
            data.lastState = direction;
            this.resolveAndOverrideStyles(data, { [direction]: overrideStyles });
    ...
    toggle(elements, executeForAll = false, recomputeIndices = true, overrideStyles, overrideBeforeComputing = this.beforeComputing) {
        this.getEnabledEntriesData(elements, executeForAll, recomputeIndices, overrideBeforeComputing)
            .forEach((data) => {
            if (!data.lastState)
                data.lastState = this.stateOf(data.element) || InOut.out;
    ...
    reloadFor(element) {
        if (this.enabled == TransitionMode.disabled
            || element.transitions.enabled == TransitionMode.disabled)
            return this;
        const data = this.findData(element);
        if (!data || data.enabled == TransitionMode.disabled)
            return this;
        const direction = data.lastState;
        const setTransition = eachEqualToAny([TransitionMode.enabled, TransitionMode.transitionOnly], this.enabled, element.transitions.enabled, data.enabled)
            && (this.duration[direction] != 0 || this.delay[direction] != 0);
        const setStyles = eachEqualToAny([TransitionMode.enabled, TransitionMode.stylesOnly], this.enabled, element.transitions.enabled, data.enabled);
        if (setTransition)
    ...
    setTransitionField(value, field) {
        ...
        [InOut.in, InOut.out].forEach(direction => this[`${direction}Transition`][field] = isInOutRecord ? value[direction] : value);
    }
    ...
    static parseMode(state) {
        if (state == true)
            return TransitionMode.enabled;
        else if (state == false)
            return TransitionMode.disabled;
        else
            return state;
    }
}
...
__decorate([
    cache()
], TurboIcon.prototype, "fetchSvg", null);
__decorate([
    observe
 ...
 * @extends TurboElement
 */
let TurboSelectEntry = TurboSelectEntry_1 = class TurboSelectEntry extends TurboElement {
    _value;
    _selected;
    _enabled;
    /**
...
class TurboSelectInputEvent extends TurboEvent {
    toggledEntry;
    values;
    constructor(toggledEntry, values, clickMode = ClickMode.left, eventInitDict) {
 ...
 * @extends TurboElement
 */
let TurboDropdownEntry = class TurboDropdownEntry extends TurboSelectEntry {
    /**
     * @description DropdownEntry constructor
...
let MarkingMenu = class MarkingMenu extends TurboSelect {
    transition;
    semiMajor;
    semiMinor;
    currentOrigin;
...
], MarkingMenu.prototype, "startAngle", null);
__decorate([
    auto({ callBefore: (value) => value - Math.PI / 2 })
], MarkingMenu.prototype, "endAngle", null);
MarkingMenu = __decorate([
